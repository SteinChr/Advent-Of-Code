...
for (let i = 0; i < input.length; i++) {
    ...
    for (let l = 0; l < 5; l++) {
        newGrid.push(...input[i][0].split(''));
        if (l != 4) newGrid.push('?');

        newGroupNumbers.push(...input[i][1].split(',').map(x => parseInt(x)));
    }

    let possibilities: Possibility[] = [{ grid: newGrid, groupNumbers: newGroupNumbers, gridIndex: 0, groupIndex: 0, counter: 1 }];
