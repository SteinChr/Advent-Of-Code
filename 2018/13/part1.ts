...
let crashFound: boolean = false;

while (!crashFound) {
    for (let c = 0; c < carts.length && !crashFound; c++) {
    ...
    for (let c1 = 0; c1 < carts.length; c1++) {
        for (let c2 = c1 + 1; c2 < carts.length; c2++) {
            if (carts[c1].x == carts[c2].x && carts[c1].y == carts[c2].y) {
                console.log(carts[c1].x.toString() + ',' + carts[c2].y.toString());
                crashFound = true;
            }
        }
    }
...
function readCarts() {
    ...
    for (let y = 0; y < input.length; y++) {
        for (let x = 0; x < input[y].length; x++) {
            if (input[y][x] == 'v') {
                carts.push({ x: x, y: y, facingDirection: 'down', nextTurningDirection: 'left' });
                input[y][x] = '|';
            } else if (input[y][x] == '^') {
                carts.push({ x: x, y: y, facingDirection: 'up', nextTurningDirection: 'left' });
                input[y][x] = '|';
            } else if (input[y][x] == '>') {
                carts.push({ x: x, y: y, facingDirection: 'right', nextTurningDirection: 'left' });
                input[y][x] = '-';
            } else if (input[y][x] == '<') {
                carts.push({ x: x, y: y, facingDirection: 'left', nextTurningDirection: 'left' });
                input[y][x] = '-';
            }
        }
    }
