...
let status = 'processing'; //processing, break, success

for (let i = 4; status != 'success'; i++) {
    status = 'processing';
    setPreValues(i);
    doCombat();
}
...
function setPreValues(attackPowerForElves: number) {
    input = readFileSync('./input.txt', 'utf-8').split('\r\n').map(x => x.split(''));
    winningPaths = [];
    nodesToCheck = [];
    removeDeadUnitNecessary = false;
    gameOverNumberOfRounds = -1;
    result = 0;

    units = [];
    for (let y = 0; y < input.length; y++) {
        for (let x = 0; x < input[y].length; x++) {
            if (input[y][x] == 'G') units.push({ group: input[y][x], x: x, y: y, attackPower: 3, hitPoints: 200 });
            if (input[y][x] == 'E') units.push({ group: input[y][x], x: x, y: y, attackPower: attackPowerForElves, hitPoints: 200 });
        }
    }
    units.sort(compare);
}
    ...
    for (let i = 0; i < possibleTargets.length; i++) {
        if (possibleTargets[i].manhattanDistance <= bestDistance) {
            ...
            if (!possibleDirections.some(x => x.distance == currentPath.distance && x.rankingDistance == currentPath.rankingDistance && x.startingDirection == currentPath.startingDirection)) {
                if (possibleDirections.every(pd => pd.distance > currentPath.distance)) {
                    possibleDirections = [];
                }
