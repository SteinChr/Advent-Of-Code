...
class Room {
    hallway: string[] = input[1].replace(/\#/g, '').split('').map(x => x.toLowerCase());
    aColumn: string[] = [input[3].charAt(3).toLowerCase(), 'd', 'd', input[2].charAt(3).toLowerCase()].filter(x => x != '.');
    bColumn: string[] = [input[3].charAt(5).toLowerCase(), 'b', 'c', input[2].charAt(5).toLowerCase()].filter(x => x != '.');
    cColumn: string[] = [input[3].charAt(7).toLowerCase(), 'a', 'b', input[2].charAt(7).toLowerCase()].filter(x => x != '.');
    dColumn: string[] = [input[3].charAt(9).toLowerCase(), 'c', 'a', input[2].charAt(9).toLowerCase()].filter(x => x != '.');
    cost: number = 0;
}
class Move {
    destination: 'a' | 'b' | 'c' | 'd';
...
function makePossibleMoves(room: Room, path: Room[]) {
    path.push(room);
    if (room.cost > minCost) return;
    if (visitedRooms.has(convertToString(room))) return;
    visitedRooms.add(convertToString(room));
    if (room.aColumn.length == roomSize && room.aColumn.every(x => x == 'a') &&
        room.bColumn.length == roomSize && room.bColumn.every(x => x == 'b') &&
        room.cColumn.length == roomSize && room.cColumn.every(x => x == 'c') &&
        room.dColumn.length == roomSize && room.dColumn.every(x => x == 'd')) {
        if (room.cost < minCost) minCost = room.cost;
    } else {
        for (let i = 0; i < room.hallway.length; i++) {
            if (room.hallway[i] != '.') {
                ...
                if (move != null) {
                    ...
                    makePossibleMoves(newRoom, [...path]);
                }
            }
        }
...
function convertToString(room: Room): string {
    return room.hallway.join('') + '/' + room.aColumn.join('') + '/' + room.bColumn.join('') + '/' + room.cColumn.join('') + '/' + room.dColumn.join('') + '/' + room.cost;
}
...
function moveToHallway(room: Room, hallwayPos: number, col: 'a' | 'b' | 'c' | 'd'): Room {
    ...
    if (col == 'a') {
        ...
        newRoom.cost += getCost(hallwayPos, newRoom.hallway[hallwayPos] as 'a' | 'b' | 'c' | 'd', newRoom.aColumn, getEntrance('a'));
    } else if (col == 'b') {
        newRoom.hallway[hallwayPos] = newRoom.bColumn.pop();
        newRoom.cost += getCost(hallwayPos, newRoom.hallway[hallwayPos] as 'a' | 'b' | 'c' | 'd', newRoom.bColumn, getEntrance('b'));
    } else if (col == 'c') {
        newRoom.hallway[hallwayPos] = newRoom.cColumn.pop();
        newRoom.cost += getCost(hallwayPos, newRoom.hallway[hallwayPos] as 'a' | 'b' | 'c' | 'd', newRoom.cColumn, getEntrance('c'));
    } else if (col == 'd') {
        newRoom.hallway[hallwayPos] = newRoom.dColumn.pop();
        newRoom.cost += getCost(hallwayPos, newRoom.hallway[hallwayPos] as 'a' | 'b' | 'c' | 'd', newRoom.dColumn, getEntrance('d'));
    }
    return newRoom;
}
...
function getCost(hallwayPos: number, entrance: 'a' | 'b' | 'c' | 'd', column: string[], fromEntrance: number = getEntrance(entrance)): number {
