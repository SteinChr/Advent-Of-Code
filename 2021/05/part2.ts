...
for (let i = 0; i < input.length; i += 4) {
    ...
    if (x1 == x2) {
        ...
        if (x1 < x2) {
            for (let x = x1; x <= x2; x++) {
                if (y1 < y2) {
                    grid[y1 + (x - x1)] = grid[y1 + (x - x1)].substr(0, x) + (parseInt(grid[y1 + (x - x1)].substr(x, 1)) + 1).toString() + grid[y1 + (x - x1)].substr(x + 1);
                } else {
                    grid[y1 - (x - x1)] = grid[y1 - (x - x1)].substr(0, x) + (parseInt(grid[y1 - (x - x1)].substr(x, 1)) + 1).toString() + grid[y1 - (x - x1)].substr(x + 1);
                }
            }
            ...
            for (let x = x2; x <= x1; x++) {
                if (y1 > y2) {
                    grid[y2 + (x - x2)] = grid[y2 + (x - x2)].substr(0, x) + (parseInt(grid[y2 + (x - x2)].substr(x, 1)) + 1).toString() + grid[y2 + (x - x2)].substr(x + 1);
                } else {
                    grid[y2 - (x - x2)] = grid[y2 - (x - x2)].substr(0, x) + (parseInt(grid[y2 - (x - x2)].substr(x, 1)) + 1).toString() + grid[y2 - (x - x2)].substr(x + 1);
                }
            }
