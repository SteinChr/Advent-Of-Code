...
function trimGrid() {
    let necessary: boolean = true;
    while (necessary) {
        for (let i = 0; i < input[0].length; i++) {
            if (input[0][i] == '#') necessary = false;
        }
        if (necessary) input.shift();
    }
    necessary = true;

    while (necessary) {
        for (let i = 0; i < input.length; i++) {
            if (input[i][0] == '#') necessary = false;
        }
        if (necessary) input.map(x => x.shift());
    }
    necessary = true;

    while (necessary) {
        for (let i = 0; i < input.length; i++) {
            if (input[i][input[0].length - 1] == '#') necessary = false;
        }
        if (necessary) input.map(x => x.pop());
    }
    necessary = true;

    while (necessary) {
        for (let i = 0; i < input[0].length; i++) {
            if (input[input.length - 1][i] == '#') necessary = false;
        }
        if (necessary) input.pop();
    }
}
