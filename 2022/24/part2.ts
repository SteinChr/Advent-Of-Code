...
let grid: string[][][] = new Array(1000);
...
for (let i = 0; i < 3; i++) {
    let currentResult = Number.MAX_VALUE;
    if (i == 1) {
        let newDestination: Position = { x: startingPosition.x, y: startingPosition.y };
        startingPosition = { x: destination.x, y: destination.y };
        destination = { x: newDestination.x, y: newDestination.y };
        paths.push({ position: { x: startingPosition.x, y: startingPosition.y }, minutes: result, manhattanDistance: manhattanDistance(startingPosition.x, startingPosition.y) });
    } else if (i == 2) {
        let newDestination: Position = { x: startingPosition.x, y: startingPosition.y };
        startingPosition = { x: destination.x, y: destination.y };
        destination = { x: newDestination.x, y: newDestination.y };
    ...
    for (let p = 0; paths.length != 0; p++) {
        ...
        if (currentPath.position.x == destination.x && currentPath.position.y == destination.y) {
            currentResult = currentPath.minutes;
        } else {
            currentPath.minutes++;
            if (currentPath.minutes + currentPath.manhattanDistance < currentResult) {
