...
let paths: Path[] = [{ pos: { x: start.x, y: start.y }, visited: '', score: 0, lastDir: { x: 1, y: 0 }, numberOfMoves: 0 }];
...
for (let i = 0; paths.length != 0; i++) {
    ...
    if (currentPath.pos.x == end.x && currentPath.pos.y == end.y) {
        if (result > currentPath.score) result = currentPath.score;
    } else {
        for (let d = 0; d < directions.length; d++) {
            if (input[currentPath.pos.y + directions[d].y][currentPath.pos.x + directions[d].x] == '.' && !currentPath.visited.includes(getStringFromPos({ x: currentPath.pos.x + directions[d].x, y: currentPath.pos.y + directions[d].y }))) {
                ...
                if (currentPath.score + costOfMove < result && currentPath.score + costOfMove < lowestCosts[currentPath.pos.y + directions[d].y][currentPath.pos.x + directions[d].x]) {
