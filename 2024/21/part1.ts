...
for (let i = 0; i < input.length; i++) {
    let oldPath: string[] = input[i];

    for (let t = 0; t < 3; t++) {
        let currentKey: string = 'A';
        let newPath: string = '';

        for (let l = 0; l < oldPath.length; l++) {
            newPath += pathsBetweenKeys.get(currentKey + '-' + oldPath[l]);
            newPath += 'A';
            currentKey = oldPath[l];
        }

        oldPath = newPath.split('');
    }    

    result += oldPath.length * parseInt(input[i].slice(0, input[i].length - 1).join(''));
}
