...
while (!programHalted) {
    let outputOfIntcode = intcode(grid[robot.yPos][robot.xPos] == 2 ? 1 : grid[robot.yPos][robot.xPos]);
    grid[robot.yPos][robot.xPos] = outputOfIntcode[0];
    updateRobot(outputOfIntcode[1]);
}

for (let i = 0; i < grid.length; i++) {
    if (grid[i].includes(0)) console.log(grid[i].join('').replace(/[02]/g, ' ').replace(/1/g, '#').trim());
}
